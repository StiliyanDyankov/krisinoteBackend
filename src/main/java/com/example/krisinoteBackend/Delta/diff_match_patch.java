...
public class diff_match_patch {
    ...
    public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
        ...
        while (thisDiff != null) {
            if (thisDiff.operation == Operation.EQUAL) {
                ...
                // sides of it.
                if (lastEquality != null && (lastEquality.length() <= Math.max(length_insertions1,
                        length_deletions1)) && (
    ...
    public void diff_cleanupSemanticLossless(LinkedList<Diff> diffs) {
        ...
        while (nextDiff != null) {
            if (prevDiff.operation == Operation.EQUAL && nextDiff.operation == Operation.EQUAL) {
                ...
                while (edit.length() != 0 && equality2.length() != 0 && edit.charAt(0) == equality2.charAt(
                        0)) {
                    equality1 += edit.charAt(0);
                    edit = edit.substring(1) + equality2.charAt(0);
                    equality2 = equality2.substring(1);
                    score = diff_cleanupSemanticScore(equality1, edit) + diff_cleanupSemanticScore(edit,
                            equality2);
            ...
            if (thisDiff.operation == Operation.EQUAL) {
                ...
                if (lastEquality != null && ((pre_ins && pre_del && post_ins && post_del) || (
                        (lastEquality.length() < Diff_EditCost / 2)
                                && ((pre_ins ? 1 : 0) + (pre_del ? 1 : 0) + (post_ins ? 1 : 0) + (post_del ? 1 : 0))
                                == 3))) {
                    //System.out.println("Splitting: '" + lastEquality + "'");
                    // Walk back to offending equality.
                    while (thisDiff != equalities.peek()) {
                        ...
                        if (both_types) {
                            ...
                            if (commonlength != 0) {
                                if (pointer.hasPrevious()) {
                                    thisDiff = pointer.previous();
                                    assert thisDiff.operation
                                            == Operation.EQUAL : "Previous diff should have been an equality.";
    ...
    public int match_main(String text, String pattern, int loc) {
        ...
        if (text.equals(pattern)) {
            ...
            // Nothing to match.
            return -1;
        } else if (loc + pattern.length() <= text.length() && text.substring(loc,
                        loc + pattern.length())
                .equals(pattern)) {
     ...
     * @param diffs Array of Diff objects for text1 to text2.
     * @return LinkedList of Patch objects.
     * @deprecated Prefer patch_make(String text1, LinkedList&lt;{@link Diff}&gt; diffs).
     */
    @Deprecated
        ...
        for (Diff aDiff : diffs) {
            ...
            switch (aDiff.operation) {
                case INSERT:
                    ...
                    postpatch_text =
                            postpatch_text.substring(0, char_count2) + aDiff.text + postpatch_text.substring(
                                    char_count2);
                    break;
                case DELETE:
                    patch.length1 += aDiff.text.length();
                    patch.diffs.add(aDiff);
                    postpatch_text = postpatch_text.substring(0, char_count2) + postpatch_text.substring(
        ...
        for (Patch aPatch : patches) {
            ...
            if (start_loc == -1) {
                ...
                if (text1.equals(text2)) {
                    ...
                    text = text.substring(0, start_loc) + diff_text2(aPatch.diffs) + text.substring(
                    ...
                    if (text1.length() > this.Match_MaxBits
                        ...
                        for (Diff aDiff : aPatch.diffs) {
                            if (aDiff.operation != Operation.EQUAL) {
                                ...
                                if (aDiff.operation == Operation.INSERT) {
                                    // Insertion
                                    text = text.substring(0, start_loc + index2) + aDiff.text + text.substring(
                                            start_loc + index2);
     ...
     * @return Linked List of Diff objects.
     */
    private LinkedList<Diff> diff_main(String text1,
                                       String text2,
                                       boolean checklines,
                                       long deadline) {
        // Check for null inputs.
     ...
     * @return Linked List of Diff objects.
     */
    private LinkedList<Diff> diff_compute(String text1,
                                          String text2,
                                          boolean checklines,
                                          long deadline) {
        LinkedList<Diff> diffs = new LinkedList<Diff>();
     ...
     * @return LinkedList of Diff objects.
     */
    private LinkedList<Diff> diff_bisectSplit(String text1,
                                              String text2,
                                              int x,
                                              int y,
                                              long deadline) {
